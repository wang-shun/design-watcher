...
/**
 * 通过版本id查询所有的运行中的容器。ps.后来根据业务需求，前端不想改代码，只能修改这个接口，变成查询所有的容器
 *
 * @author xianghao
 * @create 2017-10-09 下午8:28
 **/

@Component("queryContainersByVersionId")
public class QueryContainersByVersionId implements IQuery{
...
//            List<V2Container> resList = containerMapper.selectAllLive(params,(qep.getPageNum()-1)*qep.getNum_per_page(),qep.getNum_per_page());
//            dbUtil.convertV2Container(resList);
//            Integer records = containerMapper.selectCountOfAllLive(params);
...
//    }
    ...
    public GridBean excuteQuery(QueryParameter parameter) throws Exception{
        try {
            ...
            QueryEnvIdParameter qep = (QueryEnvIdParameter)parameter;
            V2Container params = this.getContainerParams(qep);
            List<V2Container> resList = containerMapper.selectByFields(params,(qep.getPageNum()-1)*qep.getNum_per_page(),qep.getNum_per_page());
            dbUtil.convertV2Container(resList);
